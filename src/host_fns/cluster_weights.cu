...
clust_prob::clust_prob(int _n) n(_n){}
...
void cluster_weights(fvec_d &big_grid, fvec_d &pi, fvec_d &tau2, fvec_d &yty, fvec_d &bxxb, int G, int V, int N, int K){
  gRepTimes<realIter>::iterator pi_iter = getGRepTimesIter(pi.begin(), pi.end(), K);
  gRepTimes<realIter>::iterator tau_iter = getGRepTimesIter(tau2.begin(), tau2.end(), K);
  gRepEach<realIter>::iterator yty_iter = getGRepEachIter(yty.begin(), yty.end(), K);
  gRepTimes<realIter>::iterator bxxb_iter = getGRepTimesIter(bxxb.begin(), bxxb.end(), K);
